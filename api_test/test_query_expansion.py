...
def generate_query_variations(query: str, llm: ChatOpenAI) -> List[str]:
    """生成查询变体"""
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a query expansion expert. Generate 3 different reformulations of the given query to improve search recall. Return ONLY a JSON array of strings."),
    ...
    try:
        ...
        if match:
            ...
            return [query] + variations[:3]  # 原查询 + 最多3个变体
...
def multi_query_search(vectorstore: Chroma, queries: List[str], k: int = 10) -> List:
    """多查询检索，合并结果"""
    doc_scores: Dict[str, Tuple] = {}  # source -> (doc, total_score, count)
    ...
    for query in queries:
        ...
        for doc, score in results:
            ...
            if source in doc_scores:
                old_doc, old_score, old_count = doc_scores[source]
                doc_scores[source] = (doc, old_score + score, old_count + 1)
            else:
                doc_scores[source] = (doc, score, 1)
    
    # 按出现次数（降序）和分数（升序）排序
    ranked = sorted(doc_scores.values(), key=lambda x: (-x[2], x[1]))
    return [(doc, score) for doc, score, _ in ranked[:k]]
...
def run_test():
    ...
    for i, test_case in enumerate(test_questions, 1):
        ...
        try:
            ...
            # 多路召回
            results = multi_query_search(vectorstore, queries, k=10)
        ...
        q_display = question[:35] + "..." if len(question) > 35 else question
        print(f"  [{i:2d}] {status} \"{q_display}\" → 排名: {rank if found else '未找到'}, 变体: {len(queries)}, 耗时: {elapsed:.2f}s")
...
if __name__ == "__main__":
    ...
    result_file = Path(__file__).parent / "recall_result_query_expansion.json"
