...
def load_vectorstore() -> Chroma:
    persist_dir = str(backend_dir / "vectordb_v2")  # 新的向量数据库
    embeddings = get_embeddings()
    
    return Chroma(
        persist_directory=persist_dir,
        embedding_function=embeddings,
        collection_name="crag_collection_v2",
    )
...
def run_recall_test():
    print("=" * 60)
    print("RAG 召回率测试 - CRAG Task 1 & 2 (优化版 v7)")
    print("向量数据库: vectordb_v2 (1490 文档块)")
    print("=" * 60)
    ...
    for i, test_case in enumerate(test_questions, 1):
        ...
        try:
            results = hybrid_search(vectorstore, question, k=10)
...
if __name__ == "__main__":
    ...
    result_file = Path(__file__).parent / "recall_result_v7.json"
