...
优化策略：
1. 增加 fetch_k 参数，获取更多候选文档
2. 调整 lambda_mult 参数，平衡相关性和多样性
3. 使用相似度分数过滤
"""
import os
import sys
...
def get_embeddings() -> OpenAIEmbeddings:
    ...
    if not api_key:
        raise ValueError("OPENAI_API_KEY 未配置 (DeepSeek 不提供 embedding API)")
...
def run_recall_test():
    print("=" * 60)
    print("RAG 召回率测试 - CRAG Task 1 & 2 (优化版 v3)")
    print("=" * 60)
    ...
    # 优化参数
    fetch_k = 50  # 获取更多候选
    lambda_mult = 0.5  # 平衡相关性和多样性 (0=最大多样性, 1=最大相关性)
    
    print(f"\n开始测试（MMR 检索优化: fetch_k={fetch_k}, lambda_mult={lambda_mult}）...")
    print("-" * 60)
    ...
    for i, test_case in enumerate(test_questions, 1):
        ...
        try:
            ...
            results = vectorstore.max_marginal_relevance_search(
                question, k=10, fetch_k=fetch_k, lambda_mult=lambda_mult
            )
    ...
    return {
        ...
        "params": {"fetch_k": fetch_k, "lambda_mult": lambda_mult}
    }
...
if __name__ == "__main__":
    ...
    result_file = Path(__file__).parent / "recall_result_v3.json"
