#!/usr/bin/env python3
"""
RAG 召回率测试 - 优化版 v9 (MRR 优化数据库)

使用新的向量数据库 (chunk=200, overlap=100)
"""
import os
import sys
import json
...
def load_vectorstore() -> Chroma:
    # 使用 MRR 优化的数据库
    persist_dir = str(backend_dir / "vectordb_mrr")
    embeddings = get_embeddings()
    
    return Chroma(
        persist_directory=persist_dir,
        embedding_function=embeddings,
        collection_name="crag_mrr",
    )
...
def run_recall_test():
    print("=" * 60)
    print("RAG 召回率测试 - CRAG Task 1 & 2 (v9 - MRR优化数据库)")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    result_file = Path(__file__).parent / "recall_result_v9.json"
