...
def run_recall_test():
    print("=" * 60)
    print("RAG 召回率测试 - CRAG Task 1 & 2 (优化版 v6 - 纯相似度)")
    print("=" * 60)
    ...
    for i, test_case in enumerate(test_questions, 1):
        ...
        # 显示分数
        score_info = ""
        if found and rank <= 10:
            doc, score = results[rank-1]
            score_info = f", 分数: {score:.4f}"
        
        q_display = question[:50] + "..." if len(question) > 50 else question
        print(f"  [{i:2d}] {status} \"{q_display}\"")
        print(f"        期望: {expected_doc}, 排名: {rank if found else '未找到'}{score_info}, 耗时: {elapsed:.3f}s")
...
if __name__ == "__main__":
    ...
    result_file = Path(__file__).parent / "recall_result_v6.json"
